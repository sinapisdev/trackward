...
function Lista({ atual, aoNovo }: { atual?: string; aoNovo: (t: 'objetivo' | 'area') => void }) {
  const { fluxos, areas, todosFluxos, minhaLista } = useDados()
  const [busca, setBusca] = useState('')

  // A lista pessoal vira um grupo dela. Ela não é uma track:
  ...
  return (
    <aside className="tk-lista">
      ...
      <div className="tk-lista-pe">
        <button className="btn" onClick={() => aoNovo('objetivo')}><Ic.plus />Novo objetivo</button>
...
function Palco({ alvo }: { alvo: { tipo: 'objetivo'; f: Fluxo } | { tipo: 'area'; a: Area } }) {
  ...
  const fluxoAtual = alvo.tipo === 'objetivo' ? alvo.f : null
  ...
  const faixas = useMemo(
    () => alvo.tipo === 'objetivo' ? [daFluxo(alvo.f, false)] : rotinas.map((f) => daFluxo(f, true)),
  ...
  const achado = useMemo(() => {
    ...
    const candidatos = alvo.tipo === 'objetivo' ? [alvo.f] : rotinas
    ...
    <section className="tk-palco">
      ...
      <div className="tk-quadro">
        <Trilho
          ...
          acao={alvo.tipo === 'objetivo' && mandaNoProcesso(eu, alvo.f, perfis)
...
export function EditorTrilha({ id }: { id?: string }) {
  const { fluxos, areas, carregando } = useDados()
  const { abrir } = useModais()
  const router = useRouter()

  const novo = (t: 'objetivo' | 'area') =>
    abrir(t === 'objetivo' ? { tipo: 'fluxo', tipoFluxo: 'esteira' } : { tipo: 'area' })

  const f = id ? fluxos.find((x) => x.id === id) : undefined
  const a = !f && id ? areas.find((x) => x.id === id) : undefined
  const alvo = f ? { tipo: 'objetivo' as const, f } : a ? { tipo: 'area' as const, a } : null
        ...
        <section className="tk-palco">
          <div className="tk-vazio">
            ...
            <p>
              ...
              tarefas de cada um e a chegada. Objetivo tem uma trilha só, rotina tem uma por
              rotina, porque área não acaba, ela dá voltas.
            </p>
            <button className="btn pri" onClick={() => novo('objetivo')}>
